#include <iostream>
#include <cassert>
#include "phoneBook.h" // Include the header file for your PhoneBook class
...
void testLoadContactsFromFile() {
    try {
        ...
        PhoneBook phoneBook;
        phoneBook.readRecordsFromFile("det.txt"); // Assuming the file is in the same directory
...
void testAddContact() {
    try {
        ...
        assertWithMessage(fetchedContacts[0]->getName() == "John Doe", "Contact not found sdfghjk");
    } catch (const std::exception& e) {
        failed_tests++;
